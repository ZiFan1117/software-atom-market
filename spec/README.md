...
6. **渐进式披露 + 机器闸**：`intent` = 一句话"实现什么"（列表/搜索层）；`description` **必填** = Markdown 详情层：四节标题 + 四张 Mermaid 图（数据流转/模块分解/时序/调用图），细则见 [`detail-convention.md`](./detail-convention.md)。校验器机器硬检，**机器过即收录，无人工评审**。
